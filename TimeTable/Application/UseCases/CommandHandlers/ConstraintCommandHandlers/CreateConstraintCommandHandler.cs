    ...
    : IRequestHandler<CreateConstraintCommand, Result<Guid>>
{
    public async Task<Result<Guid>> Handle(CreateConstraintCommand request, CancellationToken cancellationToken)
    {
        ...
        var timetableResult = await timetableRepository.GetByIdAsync(request.TimetableId);
        ...
        foreach (var room in roomRepository.GetAllAsync(timetable.UserEmail).Result.Data)
        {
            roomDictionary[room.Id] = room.Name;
        }
        
        foreach (var group in groupRepository.GetAllAsync(timetable.UserEmail).Result.Data)
        {
            groupDictionary[group.Id] = group.Name;
        }
        
        foreach (var course in courseRepository.GetAllAsync(timetable.UserEmail).Result.Data)
        {
        ...
        var professorId = professorIds.Find(p =>
            professorRepository.GetByIdAsync(p).Result.Data.Email == request.ProfessorEmail);
    ...
    private static ChatGptConstraintResponse ExtractConstraintResponse(string jsonString)
    {
