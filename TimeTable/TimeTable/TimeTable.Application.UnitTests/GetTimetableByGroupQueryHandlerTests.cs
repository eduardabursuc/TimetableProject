...
{
    public class GetTimetableByGroupQueryHandlerTests
    {
        private readonly ITimetableRepository _repository = Substitute.For<ITimetableRepository>();
        private readonly IMapper _mapper = Substitute.For<IMapper>();

        [Fact]
        public async Task Given_GetTimetableByGroupQueryHandler_When_HandleIsCalled_Then_TimetableShouldBeReturned()
        {
            // Arrange
            var timetable = GenerateTimetable();
            _repository.GetByGroupAsync(Arg.Any<Guid>(), Arg.Any<Guid>()).Returns(Result<Timetable>.Success(timetable));

            var query = new GetTimetableByGroupQuery { Id = timetable.Id, GroupId = timetable.Events.First().GroupId };
            var timetableDto = GenerateTimetableDto(timetable);
            _mapper.Map<TimetableDto>(timetable).Returns(timetableDto);

            var handler = new GetTimetableByGroupQueryHandler(_repository, _mapper);
        ...
        [Fact]
        public async Task Given_GetTimetableByGroupQueryHandler_When_TimetableNotFound_Then_FailureResultShouldBeReturned()
        {
            // Arrange
            _repository.GetByGroupAsync(Arg.Any<Guid>(), Arg.Any<Guid>()).Returns(Result<Timetable>.Failure("Timetable not found"));

            var query = new GetTimetableByGroupQuery { Id = Guid.NewGuid(), GroupId = Guid.NewGuid() };
            var handler = new GetTimetableByGroupQueryHandler(_repository, _mapper);
        ...
        [Fact]
        public async Task Given_GetTimetableByGroupQueryHandler_When_MappingFails_Then_ExceptionShouldBeThrown()
        {
            // Arrange
            var timetable = GenerateTimetable();
            _repository.GetByGroupAsync(Arg.Any<Guid>(), Arg.Any<Guid>()).Returns(Result<Timetable>.Success(timetable));
